            ...
            if i in self.out_indices:
                ...
                out = out.view(-1, *out_hw_shape,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
                # out = out.view(-1, *out_hw_shape,
                #                self.num_features[i]).contiguous()
                outs.append(out)
        x = outs[-1]
        return x 
        # modded by jwd 20230523
        # x = self.avgpool(outs[-1])
        # x = torch.flatten(x, 1)
        # return x, outs

@BACKBONE.register("swin_b_solider")
def swin_base_patch4_window7_224(img_size=(256, 128),drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    state_dict = torch.load('weights/swin_base_solider.pth', map_location='cpu')['student']
...
@BACKBONE.register("swin_s_solider")
...
@BACKBONE.register("swin_t_solider")
def swin_tiny_patch4_window7_224(img_size=224,drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    state_dict = torch.load('weights/swin_tiny_solider.pth', map_location='cpu')['student']
    new_state_dict = OrderedDict()
    for k,v in state_dict.items():
        if k.startswith('head.'):
            continue
