...
def vit_base_patch16_224(nattr=1, pretrained=True, **kwargs):
    model = VisionTransformer(nattr, img_size=(224,224), patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = default_cfgs['vit_base_patch16_224']
    # pretrain_path = '/public/share/others/challenge_dataset/weights/hap_ckpt_default_pretrain_pose_mae_vit_base_patch16_LUPersonPose_399.pth'
    pretrain_path='/public/share/others/challenge_dataset/weights/hap_vit_base_patch16_PPAR_SSL_100.pth'
...
def vit_base_patch16_224(nattr=1, pretrained=True, **kwargs):
    ...
    if pretrained:
        load_pretrained(
            model,
            pretrain='weights/v100_32g_vitbase_size224_lr1e3_stepLRx3_bmp1_adafactor_wd01_clip05_layerdecay075_lpe_peddet_citypersons_LSA_reduct8_tbn1_heads2_gate1_peddetShareDecoder_exp3_setting_SharePosEmbed.pth',
            
            )

    return model

@BACKBONE.register("vit_b_dinov1")
def vit_base_patch16_224(nattr=1, pretrained=True, **kwargs):
    model = VisionTransformer(nattr, img_size=(224,224), patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = default_cfgs['vit_base_patch16_224']
    if pretrained:
        load_pretrained(
            model,
            pretrain='weights/dino_vitbase16_pretrain.pth',)
    return model
@BACKBONE.register("vit_b_dinov2")
def vit_base_patch14_224(nattr=1, pretrained=True, **kwargs):
    model = VisionTransformer(nattr, img_size=(224,224), patch_size=14, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = default_cfgs['vit_base_patch16_224']
    if pretrained:
        load_pretrained(
            model,
            pretrain='weights/dinov2_vitb14_pretrain.pth',
            strict=False)
    return model
...
#     return model
#
@BACKBONE.register("vit_l")
def vit_large_patch16_224(nattr=1, pretrained=False, **kwargs):
    model = VisionTransformer(nattr, img_size=(256, 192),
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = default_cfgs['vit_large_patch16_224']
    if pretrained:
        load_pretrained(model, pretrain='weights/vitlarge_size224_lr5e4_stepLRx3_bmp1_adafactor_clip05_wd01_layerdecay08_lpe_LSA_reduct8_tbn1_heads2_gate1_peddetDPR02_peddetShareDecoder_exp3_setting_SharePosEmbed.pth')
    return model
#
#
...
def load_pretrained(model, pretrain, strict=True):
    ...
    state_dict = state_dict['model'] #not be used in vit dinov1
    ...
    for k, v in state_dict.items():
        ...
        if k == 'pos_embed' and v.shape != model.pos_embed.shape:
            ...
            # pos_embed = resize_pos_embed_v2(pos_embed,(224,224),(224,224),mode='bilinear')

            state_dict = interpolate_pos_embed(model, state_dict, (16, 8), 'pos_embed')
...
def resize_pos_embed_v2(pos_embed,input_shape,pos_shape,mode):
    assert pos_embed.ndim == 3, "shape of pos_embed must be [B, L, C]"
    pos_h, pos_w = pos_shape[0] // 14, pos_shape[1] // 14
