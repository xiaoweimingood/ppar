...
def build_position_encoding(hidden_dim=2048, position_embedding='sine', img_h=256, img_w=256):
    N_steps = hidden_dim // 2
    downsample_ratio = 32

    if position_embedding in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        assert img_h % 32 == 0, "img_h ({}) % 32 != 0".format(img_h)
        assert img_w % 32 == 0, "img_w ({}) % 32 != 0".format(img_w)
        position_embedding = PositionEmbeddingSine(N_steps, 
                                                   normalize=True, 
                                                   maxH=img_h // downsample_ratio, 
                                                   maxW=img_w // downsample_ratio)
