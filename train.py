...
from models.model_factory import build_loss, build_classifier, build_backbone, FeatClassifier

from tools.function import get_model_log_path, get_reload_weight, seperate_weight_decay
from tools.utils import time_str, save_ckpt, ReDirectSTD, set_seed, str2bool, save_ckpt_other

from loguru import logger

torch.autograd.set_detect_anomaly(True)

global_time_str = time_str()

def main(cfg, args):
    set_seed(605)
    exp_dir = os.path.join('exp_result', cfg.DATASET.NAME)
    new_name = cfg.NAME.format(cfg.BACKBONE.TYPE, 
                               cfg.CLASSIFIER.NAME, 
                               cfg.TRAIN.USE_LABEL_EMBEDDING, 
                               cfg.TRAIN.USE_VTB,
                               cfg.TRAIN.BATCH_SIZE,
                               cfg.TRAIN.MAX_EPOCH,
                               cfg.LOSS.TYPE,
                               cfg.TRAIN.OPTIMIZER.TYPE,
                               cfg.TRAIN.LR_SCHEDULER.TYPE,
                               cfg.TRAIN.LR_SCHEDULER.LR_FT,
                               cfg.TRAIN.LR_SCHEDULER.LR_NEW)
    model_dir, log_dir = get_model_log_path(exp_dir, new_name)
    stdout_file = os.path.join(log_dir, f'stdout_{global_time_str}.txt')
    model_path = os.path.join(model_dir, 'ckpt_{}_{}_{}.pth')
    

    writer = None
    if cfg.VIS.TENSORBOARD.ENABLE:
        current_time = datetime.now().strftime('%b%d_%H-%M-%S')
        writer_dir = os.path.join(exp_dir, new_name, 'runs', current_time)
    ...
    model = FeatClassifier(backbone,
                           classifier,
                           bn_wd=cfg.TRAIN.BN_WD,
                           dim=c_output,
                           label_embedding=cfg.TRAIN.USE_LABEL_EMBEDDING,
                           use_vtb=cfg.TRAIN.USE_VTB,
                           )
    if args.local_rank == 0:
        print(
            f"backbone: {cfg.BACKBONE.TYPE}, classifier: {cfg.CLASSIFIER.NAME}"
        )
        print(f"model_name: {new_name}")
    ...
    criterion = build_loss(cfg.LOSS.TYPE)(sample_weight=label_ratio,
                                          scale=cfg.CLASSIFIER.SCALE,
    ...
    if args.local_rank == 0:
        print(
            f'{new_name},  best_ma_metrc : {max_ma} in epoch{best_epoch}')


def trainer(cfg, args, epoch, model, model_ema, train_loader, valid_loader,
            criterion, optimizer, lr_scheduler, model_path, loss_w,
            tb_writer):
    max_ma = float(-np.inf)
    best_epoch = 0
    best_metric_log = defaultdict(lambda: float('-inf'))
    result_list = defaultdict()
    result_path = model_path.replace('pth', 'pkl')
    ...
    for e in range(epoch):
        ...
        if cfg.METRIC.TYPE == 'pedestrian':
            ...
            if args.local_rank == 0:
                ...
                print(
                    f'Evaluation on train set, train losses {train_loss}\n',
                    'ma: {:.4f}, label_f1: {:.4f}, pos_recall: {:.4f} , neg_recall: {:.4f} \n'
                    .format(train_result.ma, np.mean(train_result.label_f1),
                            np.mean(train_result.label_pos_recall),
                            np.mean(train_result.label_neg_recall)),
                    'Acc: {:.4f}, Prec: {:.4f}, Rec: {:.4f}, F1: {:.4f}'.
                    format(train_result.instance_acc,
                           train_result.instance_prec,
                           train_result.instance_recall,
                           train_result.instance_f1))

                print(
                    f'Evaluation on test set, valid losses {valid_loss}\n',
                    'ma: {:.4f}, label_f1: {:.4f}, pos_recall: {:.4f} , neg_recall: {:.4f} \n'
                    .format(valid_result.ma, np.mean(valid_result.label_f1),
                            np.mean(valid_result.label_pos_recall),
                            np.mean(valid_result.label_neg_recall)),
                    'Acc: {:.4f}, Prec: {:.4f}, Rec: {:.4f}, F1: {:.4f}'.
                    format(valid_result.instance_acc,
                           valid_result.instance_prec,
                           valid_result.instance_recall,
                           valid_result.instance_f1))
                print('Label Acc\n', valid_result.label_ma.tolist())
                print(f'{time_str()}')
                print('-' * 60)

                tb_visualizer_pedes(tb_writer, lr, e, train_loss, valid_loss,
                                    train_result, valid_result, train_gt,
                                    valid_gt, train_loss_mtr, valid_loss_mtr,
                                    model, train_loader.dataset.attr_id)


                for metric in [
                        'ma', 'label_f1', 'label_pos_recall', 'label_neg_recall',
                        'instance_acc', 'instance_prec', 'instance_recall',
                        'instance_f1'
                ]:
                    metric_v = eval(f'metric_{metric}')
                    if metric_v > best_metric_log[metric]:
                        logger.warning(
                            f'best metric {metric} value {best_metric_log[metric]} -> {round(metric_v, 5)} updated at epoch {e}'
                        )
                        best_metric_log[metric] = metric_v

                        model_full_path = model_path.format(
                            global_time_str, metric, round(metric_v * 100, 3))
                        
                        # delete old ckpt
                        model_del_path = model_path.format(global_time_str, metric, '**')
                        for file in glob.glob(model_del_path):
                            os.remove(file)

                        save_ckpt(model, model_full_path, e, metric)
                        if metric == 'ma':
                            best_epoch = e
                            max_ma = metric_v
                print(f'best ma {max_ma} at ep {best_epoch}')
                result_list[e] = {
            ...
            if args.local_rank == 0:
                print(
                    ...
                    'CF1: {:.4f}'.format(train_metric.map, train_metric.OP,
                                         train_metric.OR, train_metric.OF1,
                                         train_metric.CP, train_metric.CR,
                                         train_metric.CF1))
                ...
                for metric in [
                    ...
                    metric_v = eval(f'valid_result.{metric}')
                    if metric_v > best_metric_log[metric]:
                        logger.warning(
                            f'best metric {metric} value {round(metric_v, 2)} updated at epoch {e}'
                        )
                        best_metric_log[metric] = metric_v
                        model_full_path = model_path.format(
                            global_time_str, metric, round(metric_v, 3))
                        save_ckpt(model, model_full_path, e, metric)
        ...
        result_full_path = result_path.format(global_time_str, e, 'result')
