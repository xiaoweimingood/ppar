...
class AsymmetricLoss(nn.Module):
    def __init__(self, sample_weight=None, size_sum=True, scale=None, tb_writer=None):
        ...
        self.gamma_pos = 1
        self.clip = 0.05
        self.disable_torch_grad_focal_loss = False
        self.eps = 1e-8
        self.smoothing = None
    ...
    def forward(self, x, y):
        ...
        if self.smoothing is not None:
            y = (1 - self.smoothing) * y + self.smoothing * (1 - y)
        ...
        ret_loss = ret_loss_m.sum(1).mean()
        return [ret_loss], [ret_loss_m]
...
@LOSSES.register("aplloss")
class APLLoss(nn.Module):
    def __init__(self, sample_weight=None, size_sum=True, scale=None, tb_writer=None, gamma_neg=4, gamma_pos=0, clip=0.05, eps=1e-8, disable_torch_grad_focal_loss=False):
